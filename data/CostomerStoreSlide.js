...
function CostomerStoreSlide() {
  ...
  const itemWidth = 160;
  const itemMargin = 10;
  const visibleItemCount = Math.floor(screenWidth / (itemWidth + itemMargin * 2));
  ...
  const handleScroll = (event) => {
    ...
    const fractionalOffset = contentOffsetX / (itemWidth + itemMargin * 2) - index;
  ...
  useEffect(() => {
    if (scrollViewRef.current && focusedIndex >= 0 && focusedIndex < news.length) {
      const offset = focusedIndex * (itemWidth + itemMargin * 2);
      const maxOffset = Math.max(0, news.length * (itemWidth + itemMargin * 2) - screenWidth);
      ...
      {news.map((item, index) => (
        <View
          ...
          style={[
            styles.square,
            { width: itemWidth },
            index === focusedIndex ? styles.focusedSquare : styles.unfocusedSquare,
          ]}
        >
        <Pressable>
          <Image style = {styles.image} source={{uri: item.imageUrl, "width":141,"height":90, "top": -10}}/>
